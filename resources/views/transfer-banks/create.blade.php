  ...
  >
    <header class="text-center">
      <h2 class="text-2xl font-bold uppercase mb-5">
        Create a Transfer Bank
      </h2>
    </header>

    <form method="POST" action="/transfer-bank" enctype="multipart/form-data">
      @csrf
      <div class="mb-6">
        <label
          for="customer_from"
        ...
        <select class="border border-gray-200 rounded p-2 w-full" id="customer_from"
          ...
          @foreach ($customers as $customer)
            <option value="{{ $customer->id }}" {{ ( $customer->id == old('customer_from')) ? 'selected' : '' }}> {{ $customer->name }} </option>
          @endforeach
        </select>
        @error('customer_from')
      ...
      <div class="mb-6">
        ...
        <select class="border border-gray-200 rounded p-2 w-full" id="customer_to"
          ...
          @foreach ($customers as $customer)
            <option value="{{ $customer->id }}" {{ ( $customer->id == old('customer_to')) ? 'selected' : '' }}> {{ $customer->name }} </option>
          @endforeach
        </select>
        @error('customer_to')
          <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
